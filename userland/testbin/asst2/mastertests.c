...
#define MAX_BUF 500
char teststr[] = "I like lemons. Also grapes and chicken ;)";
char buf[MAX_BUF];
...
main(int argc, char * argv[])
{
        ...
        printf("**********\n* opening new file \"mastertest.file\"\n");
        fd = open("mastertest.file", O_RDWR | O_CREAT, 0700); /* mode u=rw in octal */
        ...
        printf("* writing test string into mastertest.file\n");
        ...
        printf("buffer now contains: %s", buf);
        ...
        printf("************ testing dup2 **************\n");
        printf("testing with 2 open files\n");
        printf("opening file 1\n");
        int f1_fd = open("dup2_file1.file", O_RDWR | O_CREAT, 0700); /* mode u=rw in octal */
        printf("file1 opened with fd: %d\n", f1_fd);
        printf("opening file 2\n");
        int f2_fd = open("dup2_file2.file", O_RDWR | O_CREAT, 0700); /* mode u=rw in octal */
        printf("file2 opened with fd: %d\n", f2_fd);
        int result = dup2(f1_fd, f2_fd);
